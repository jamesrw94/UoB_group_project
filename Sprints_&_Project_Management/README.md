...
Then, we added a ‘dev’ branch to make code changes to. Each team member made a branch from ‘dev’ (e.g. “joe’s branch”), for which they could make changes as per the technical worksheets. Clearly, we were not performing proper CI as we were not committing regularly to the 'dev' branch and our own branches became quite different from one another.
