...
## The audience
The potential demographic for our proposed solution is best represented as adults who regularly consume news media with an interest in how news is portrayed. Whilst the solution could be tailored to appeal to more specific demographics, we chose to keep the concept broad for the outset. Doing so allows us to use a user-testing process further down the line to tailor our application to more granular audiences at the time, should they present themselves as more enthusiastic users. In the meantime, we aim to keep the applications potential demographic as broad as possible to ensure maximum inclusion and avoid excluding any unforeseen user groups the application could benefit. Although the goal impact of our solution will be to educate and entertain our users, we move forward from this point aware that we are examining our users, and could potentially place them under stress and possibly embarrass them. We hope that any negative reactions/experiences will become apparent in user testing throughout the development process.
